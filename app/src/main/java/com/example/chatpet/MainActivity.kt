...
// LlmUiState (Sealed class for UI states)
sealed class LlmUiState {
    object Idle : LlmUiState()
    object Loading : LlmUiState()
    data class Success(val resultText: String) : LlmUiState()
    data class Error(val errorMessage: String) : LlmUiState()
}
...
fun MainScreen(
    ...
    val uiState by chatViewModel.uiState // Observe the state
